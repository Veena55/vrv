    ...
    return (
        <div className='p-5'>
            ...
            {roles && roles.map((role, index) => {
                return (
                    <div key={role.id} className={`grid grid-cols-[100px_.5fr_1fr_.8fr] items-center py-2 px-5 rounded-lg ${index % 2 == 0 ? 'bg-white' : 'bg-light'}`}>
                        <div>{index + 1}</div>
                        <div>{role.roleName}</div>
                        <div>{role.updatedAt}</div>
                        <div className='flex gap-2 items-center'>
                            <FontAwesomeIcon icon={faEdit} className='text-theme' onClick={() => handleModal(role.id, "edit")} />
                            <FontAwesomeIcon icon={faTrashCan} className='text-red-500' onClick={() => handleModal(role.id, "role")} />
                        </div>
                    </div>
